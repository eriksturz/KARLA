...
function getDatafromTrip($mysqli, $tripID, $userID)
{
    ...
    $distance = null;
    $co2_emissions = null;
    $stmt = $mysqli->prepare("SELECT distance, co2_emissions FROM trip WHERE tripID = ? AND userID = ?");
    $stmt->bind_param("ii", $tripID, $userID);
    if (!$stmt->execute()) {
        echo json_encode(array('error' => 'Error executing query: ' . $stmt->error));
        exit;
    }
    $stmt->bind_result($distance, $co2_emissions);
    if (!$stmt->fetch()) {
    ...
    error_log("Distance: " . $distance . ", CO2 output: " . $co2_emissions);
    return array('distance' => $distance, 'co2_emissions' => $co2_emissions);
}
